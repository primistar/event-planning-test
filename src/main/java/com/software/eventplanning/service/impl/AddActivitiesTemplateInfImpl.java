...
@Service
public class AddActivitiesTemplateInfImpl extends ServiceImpl<ActivitiesTemplateMapper, Template>
        implements IAddActivitiesTemplateInfService {
    ...
    @Override
    public IPage<Template> list(Integer pageNum, Integer pageSize, Integer templateId, String key) {
        IPage<Template> page = new Page<>(pageNum, pageSize);
        QueryWrapper<Template> wrapper = new QueryWrapper<>();
        wrapper.eq(templateId != null && templateId.intValue() > 0, "template_id", templateId);
        wrapper.orderByDesc("activity_id");
        wrapper.like(Strings.isNotEmpty(key), "host_name", key);
